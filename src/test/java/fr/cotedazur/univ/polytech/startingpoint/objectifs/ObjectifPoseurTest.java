...
class ObjectifPoseurTest {
    ...
    void setUp() {
        ...
        // 2. On crée le GameState (qui crée lui-même son Plateau vide)
        gameState = new GameState(List.of(bot));

        // 3. On crée l'objectif à tester
        // Rappel : ObjectifPoseur(int nombreRequis, Couleur couleur)
        // (Les points sont fixés à 2 dans la classe, pas dans le constructeur)
        objectifVert = new ObjectifPoseur(3, Couleur.VERT);
    }
    ...
    void testObjectifValide() {
        ...
        plateauDuJeu.placerParcelle(new Parcelle(Couleur.VERT), new Position(0, 1, -1));

        // 4. Validation finale (Signature : valider(GameState, Bot))
